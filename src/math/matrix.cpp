...
Matrix Matrix::hadamaard_product(const Matrix &m) {
  if (!(this->num_rows == m.num_rows && this->num_cols == m.num_cols)) {
    throw std::invalid_argument(
        "DIMENSIONS OF THE MATRICES MUST MATCH! (HADAMARD PROD)");
  }
...
double *Matrix::operator[](int row) { return &data[row * num_cols]; }
  ...
  for (std::uint32_t row = 0; row < num_rows; row++) {
    for (std::uint32_t col = 0; col < num_cols; col++) {
      sum.data[row * num_cols + col] =
          this->data[row * num_cols + col] + m.data[row * num_cols + col];
    }
  }
...
std::size_t Matrix::num_elements() { return this->num_rows * this->num_cols; }
