...
private:
   ...
   */
  void update_init_weights_ReLU();

  /**
   * @brief Gradient is d(Loss), activation for ReLU : gradient[i][j] = 0 if
   * pred[i][j] < 0
  ...
  /**
   * @brief Gradient is d(Loss), activation for sigmoid : gradient[i][j] =
   * sigmoid(gradient[i][j])*(1 - gradient[i][j])
   * @param grad_output gradient output after forward pass
   */
...
public:
   ...
   * weights, biases, inputs are used by the backprop and hence are cached at
   * each layer
   ...
   */
  NeuralNetwork(std::size_t input, std::size_t output, std::size_t hiddenL,
                std::size_t batchS, double lr);
   ...
   * @param batchS batch_size
   */
  NeuralNetwork(std::size_t input, std::size_t output,
                std::vector<int> hidden_sz, std::size_t batchS, double lr);
   ...
   */
  double cross_entropy_loss(Matrix &pred, Matrix &actual);
